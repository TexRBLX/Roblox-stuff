...
function EspObject:Construct()
	self.partCache = {}; -- [MODIFIED] Replaced charCache with a more versatile part cache
	self.childCount = 0;
	self.bin = {};
	self.drawings = {
		box3d = {
    ...
    for i = 1, #SKELETON_CONNECTIONS do
        ...
        self.drawings.skeletonOutline[i] = self:_create("Line", { Thickness = 3, Visible = false });
    end
...
function EspObject:Update()
	...
	if self.onScreen then
		...
		if self.childCount ~= #children then
			clear(self.partCache);
			for i = 1, #children do
        ...
        local bboxParts = {};
        for name, part in pairs(self.partCache) do
            if isBodyPart(name) then
                table.insert(bboxParts, part);
            end
        end
		self.corners = calculateCorners(getBoundingBox(bboxParts));

        if self.options and self.options.skeleton then
            self.skeletonScreenPoints = self.skeletonScreenPoints or {};
            clear(self.skeletonScreenPoints);
            for name, part in pairs(self.partCache) do
                local point, visible, z = worldToScreen(part.Position);
                if z > 0 then
                    self.skeletonScreenPoints[name] = point;
                end
            end
        end
...
function EspObject:Render()
    ...
    local skeletonEnabled = enabled and onScreen and options.skeleton and self.skeletonScreenPoints;
    local skeletonLines = self.drawings.skeleton;
    local skeletonOutlineLines = self.drawings.skeletonOutline;

    for i = 1, #SKELETON_CONNECTIONS do
        local connection = SKELETON_CONNECTIONS[i];
        local line = skeletonLines[i];
        local outline = skeletonOutlineLines[i];
