...
-- [MODIFIED] Define the targetGui where all UI elements will be placed.
local playerGui = LocalPlayer:WaitForChild("PlayerGui")
local targetGui = playerGui:FindFirstChild("Chat") or playerGui:FindFirstChildOfClass("ScreenGui")
assert(targetGui, "Obsidian: Could not find a suitable targetGui to parent UI to.")
...
-- [MODIFIED] This section has been changed to use the targetGui instead of creating new ScreenGuis.

-- We are parenting everything to a container inside the user's targetGui,
-- so these functions are no longer needed in their original form.
local function SafeParentUI() end
local function ParentUI() end

-- Create a single container for all of the library's UI elements.
-- This container will be parented to the targetGui.
local ObsidianContainer = New("Frame", {
    Name = "ObsidianContainer",
    Size = UDim2.fromScale(1, 1),
    BackgroundTransparency = 1,
    Parent = targetGui,
    ZIndex = 998, -- Set a high ZIndex to render on top of most game UI.
})
Library.ScreenGui = ObsidianContainer

-- When any descendant is removed from our container, clean it from the registries.
ObsidianContainer.DescendantRemoving:Connect(function(Instance)
...
do
    NotificationArea = New("Frame", {
        ...
        ZIndex = 1000, -- [MODIFIED] Ensure notifications appear on top of everything.
        Parent = Library.ScreenGui,
    })
...
function Library:MakeDraggable(UI: GuiObject, DragFrame: GuiObject, IgnoreToggled: boolean?, IsMainWindow: boolean?)
    ...
    Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input: InputObject)
        if
            ...
            or not (Library.ScreenGui and Library.ScreenGui.Parent)
        then
            Dragging = false
            if Changed and Changed.Connected then
                Changed:Disconnect()
...
function Library:MakeResizable(UI: GuiObject, DragFrame: GuiObject, Callback: () -> ()?)
    ...
    Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input: InputObject)
        if not UI.Visible or not (Library.ScreenGui and Library.ScreenGui.Parent) then
            Dragging = false
            if Changed and Changed.Connected then
                Changed:Disconnect()
                Changed = nil
...
function Library:AddDraggableMenu(Name: string)
    local Background = Library:MakeOutline(Library.ScreenGui, Library.CornerRadius, 10)
...
--// Watermark \\--
do
    local WatermarkBackground = Library:MakeOutline(Library.ScreenGui, Library.CornerRadius, 10)
...
function Library:Unload()
    ...
    -- [MODIFIED] Destroy the main container frame instead of the old ScreenGuis
    if Library.ScreenGui and Library.ScreenGui:IsA("Frame") then
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        MainFrame = New("Frame", {
            ...
            Visible = false,
            ZIndex = 2, -- [MODIFIED] Set ZIndex to be higher than the modal blocker.
            Parent = Library.ScreenGui,
